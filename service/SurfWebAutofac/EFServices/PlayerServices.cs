...
public class PlayerServices : BaseServices<PlayerModel>, IPlayerServices
{
    ...
    public PlayerServices(IBaseRepository<MapModel> mapRepository,
        IBaseRepository<PlayerCompleteModel> playerCompleteRepository,
        IBaseRepository<PlayerModel> playerRepository, IMapServices mapServices) : base(playerRepository)
    {
    ...
    {
        var result = await _playerRepository.Select(t => new PlayerInfoDto
        {
            Id = t.Id,
            Name = t.Name,
            Integral = t.Integral,
        ...
        {
            //积分排行
            result.IntegralRanking = await _playerRepository
                .Where(t => t.Integral > result.Integral)
                .CountAsync() + 1;
            //地图完成数排行
            result.SucceesRanking = await _playerRepository
                .Where(t => t.SucceesNumber > result.SucceesNumber)
                .CountAsync() + 1;
            //主线排行
            result.WRRanking = await _playerRepository
                .Where(t => t.WRNumber > result.WRNumber)
                .CountAsync() + 1;
            //奖励排行
            result.BWRanking = await _playerRepository
                .Where(t => t.BWRNumber > result.BWRNumber)
                .CountAsync() + 1;
            //阶段排行
            result.SWRanking = await _playerRepository
        ...
        string difficulty, int pageIndex)
    {
        ...
        //3.查询wr信息
        var mapWrList = (await _mapServices.GetMapWrList(recordType)).Where(t =>
    ...
    public async Task UpdateStatsNumber()
    {
        ...
        var playerSucceesIdList = new List<long>();
    ...
    public async Task ChangeInfo(List<PlayerModel> changeList)
    {
        ...
        //修改冗余字段名字
        var list = await _playerCompleteRepository.Where(t => changeList.Select(a => a.Id).Contains(t.PlayerId))
            .ToListAsync();
    ...
    {
        return await _playerRepository.Where(t => names.Contains(t.Name)).ToListAsync();
    }

    /// <summary>
    ///     获取玩家WRCount
    /// </summary>
    public async Task<int> GetPlayerWRCount(long id, RecordTypeEnum recordType, string difficulty)
    {
        return (await GetPlayerWRQueryable(id, recordType, difficulty)).Count();
    }

    /// <summary>
    ///     获取玩家WRList
    /// </summary>
    public async Task<List<PlayerWRDto>> GetPlayerWRList(long id, RecordTypeEnum recordType, string difficulty,
        int pageIndex)
    {
        return (await GetPlayerWRQueryable(id, recordType, difficulty))
            .OrderByDescending(t => t.Key)
            .PageData(pageIndex, 10)
    ...
    private async Task<IEnumerable<IGrouping<long, MapWrCache>>> GetPlayerWRQueryable(long id,
        RecordTypeEnum recordType,
        string difficulty)
    {
        return (await _mapServices.GetMapWrList(recordType))
            .OrderByDescending(t => t.Date)
            .Where(t => t.PlayerId == id && t.Type == recordType && t.Difficulty == difficulty)
            .GroupBy(t => t.MapId);
    }

    private IQueryable<IGrouping<long, PlayerCompleteModel>> GetPlayerSucceesQueryable(long id,
