...
namespace Common.Db.EFCore.Repository;
...
public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : BaseEntity
{
    ...
    {
        if (entity == null) throw new Exception("[新增]入参为空");
        entity.Id = _worker.NextId();
    ...
    public List<TEntity> Inserts(IEnumerable<TEntity> entitys)
    {
        ...
        foreach (var entity in entitys) result.Add(Insert(entity));
        return result;
    }
    ...
    {
        if (entity == null) throw new Exception("[修改]入参为空");
        //跟踪查询
        var formerEntity = _dbContext.Set<TEntity>().FirstOrDefault(t => t.Id == entity.Id);
        if (formerEntity == null) throw new Exception($"[修改]无ID为{entity.Id}的数据");
        //修改数据
        foreach (var property in formerEntity.GetType().GetProperties())
        {
            var value2 = property.GetValue(entity);
    ...
    {
        var list = _dbContext.Set<TEntity>().IgnoreQueryFilters()
            .Where(t => t.IsDelete == 0 && entitys.Select(a => a.Id).Contains(t.Id)).ToList();
    ...
    public void Delete(long id)
    {
        ...
        if (formerEntity == null) throw new Exception($"[删除]无ID为{id}的数据");
        formerEntity.UpDateTime = DateTime.Now;
    ...
    {
        var formerEntity = _dbContext.Set<TEntity>().FirstOrDefault(t => t.Id == entity.Id);
        if (formerEntity == null) throw new Exception($"[删除]无ID为{entity.Id}的数据");
        formerEntity.UpDateTime = DateTime.Now;
        formerEntity.IsDelete = 1;
    }

    public void Deletes(IEnumerable<TEntity> entitys)
    {
        var formerEntitys = _dbContext.Set<TEntity>().Where(t => entitys.Select(a => a.Id).Contains(t.Id)).ToList();
        if (!formerEntitys.Any()) throw new Exception("[删除]无删除的数据");
        foreach (var formerEntity in formerEntitys)
        {
            formerEntity.UpDateTime = DateTime.Now;
            formerEntity.IsDelete = 1;
        }
