...
public class PlayerServices : BaseServices<PlayerModel>, IPlayerServices
{
    ...
    {
        return await _repository.Where(t => names.Contains(t.Name)).ToListAsync();
    }

    private IEnumerable<IGrouping<string, MapWrCache>> GetPlayerWRQueryable(string id, RecordTypeEnum recordType,
        string difficulty)
    {
    ...
    private IQueryable<IGrouping<string?, PlayerCompleteModel>> GetPlayerSucceesQueryable(string id,
