...
namespace Services.Services
{
    public class MapServices : BaseServices<MapModel>, IMapServices
    {
        ...
        /// 获取地图前100数量
        /// </summary>
        public async Task<int> GetMapTop100Count(string id, RecordTypeEnum recordType, int? stage)
        {
        ...
        /// 获取地图前100
        /// </summary>
        public async Task<List<MapTop100Dto>> GetMapTop100List(string id, RecordTypeEnum recordType, int? stage, int pageIndex)
        {
            var mapWr = _dataCache.MapSnapshot.MapWrCaches
            ...
            if (result.Any())
            {
                for (int i = 0; i < result.Count; i++)
                {
                    ...
                    if (mapWr != null)
                    {
                        result[i].GapTime = mapWr.Time - result[i].Time;
                    }
                }
            }
            return result;
        }
        private IQueryable<PlayerCompleteModel> GetMapTop100Queryable(string id, RecordTypeEnum recordType, int? stage)
        {
