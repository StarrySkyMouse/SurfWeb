  ...
  版本: {Version}";
        ...
        public static List<PlayerInfo> QueryPlayers(string ip, int port, int timeout = 3000)
        {
            ...
            using (var udp = new UdpClient())
            {
                ...
                // 1. 获取挑战值
                byte[] challenge = GetChallengeValue(udp, endPoint);
        ...
        public static ServerInfo QueryServer(string ip, int port, int timeout = 3000)
        {
            ...
            using (var udp = new UdpClient())
            {
                ...
                if (response.Length >= 5 && response[4] == 0x41) // 'A' 表示需要挑战值
                {
                    ...
                    // 构造带挑战值的新请求
                    byte[] newRequest = new byte[request.Length + 4];
        ...
        private static string GetServerType(byte type) => type switch
        {
            0x64 => "专用服务器",
            0x6C => "监听服务器",
            0x70 => "SourceTV中转",
        ...
        private static string GetEnvironment(byte env) => env switch
        {
            0x77 => "Windows",
            0x6C => "Linux",
            0x6D => "macOS",
        ...
        private class ResponseReader
        {
            ...
            {
                short value = (short)(_data[_index] | (_data[_index + 1] << 8));
                _index += 2;
                return value;
            }
            ...
            {
                CheckBounds(4);
                int value = _data[_index] | (_data[_index + 1] << 8) |
            ...
            public string ReadString()
            {
                ...
                string result = Encoding.UTF8.GetString(_data, start, _index - start);
                _index++; // 跳过终止符
                return result;
            }
